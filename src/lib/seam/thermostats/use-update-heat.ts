...
type UseUpdateHeatData = Record<string, unknown>
type UseUpdateHeatMutationParams = ThermostatHeatRequest

export function useUpdateHeat(): UseMutationResult<
  UseUpdateHeatData,
  SeamError,
  UseUpdateHeatMutationParams
> {
  const { client } = useSeamClient()
  const queryClient = useQueryClient()

  return useMutation<UseUpdateHeatData, SeamError, UseUpdateHeatMutationParams>(
    {
      mutationFn: async (mutationParams: UseUpdateHeatMutationParams) => {
...
function getUpdatedThermostat(
  ...
  variables: UseUpdateHeatMutationParams
): ThermostatDevice {
  return {
    ...thermostat,
    properties: {
